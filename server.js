...
app.post("/app/new/user", function (req, res) {
	...
	const insert = stmt.run("Yang", md5("1234"));
...
app.patch("/app/update/user/:id", (req, res) => {
	const user = "Yang";
	const pass = "4567";
	const stmt = db.prepare("UPDATE userinfo SET user = COALESCE(?,user), pass = COALESCE(?,pass) WHERE id = ?").run(user, md5(pass), req.params.id);
	res.status(200).json({ "message": `User ${req.params.id} successfully updated` });
});
